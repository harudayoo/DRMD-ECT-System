<template>
  ...
  >
    <div class="bg-white p-8 rounded-lg shadow-xl w-full max-w-2xl">
      <h2 class="text-2xl font-bold mb-4 text-center">Generate New CDR</h2>
      <form @submit.prevent="validateAndSubmit">
        <div class="grid grid-cols-2 gap-4">
          <!-- CDR Name Input -->
          <div class="col-span-2">
            <label for="cdrName" class="block mb-2 font-medium">CDR Name</label>
            <input
              type="text"
              id="cdrName"
            ...
            />
            <div
              v-if="form.errors.cdrName || validationErrors.cdrName"
          ...
          <!-- Payroll Selection -->
          <div class="col-span-2">
            <label for="payrollID" class="block mb-2 font-medium">Payroll</label>
            <div class="relative">
              <input
                type="text"
                ...
                :disabled="form.processing"
              />
              <div v-if="isLoadingPayrolls" class="absolute right-3 top-2">
                <svg
            ...
            <div
              v-if="form.errors.payrollID || validationErrors.payrollID"
              class="text-sm text-red-500 mt-1"
            >
              {{ form.errors.payrollID || validationErrors.payrollID }}
            </div>
          </div>
        </div>
...
// Methods
const searchPayrolls = async (search: string) => {
  if (!search) {
  ...
  try {
    ...
    const response = await axios.get(route("cdr.searchPayrolls", { search }));
    filteredPayrolls.value = response.data;
  } catch (error) {
    console.error("Error searching payrolls:", error);
    // Use type assertion to add generalError
    form.errors.generalError = "Failed to search payrolls. Please try again.";
  } finally {
...
const fetchPayrolls = async () => {
  try {
    ...
    form.errors.generalError = "Failed to fetch payrolls. Please try again.";
  } finally {
    isLoadingPayrolls.value = false;
  }
};
