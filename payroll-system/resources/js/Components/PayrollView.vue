<template>
    <div class="relative">
        ...
        >
            ...
            <div
                v-if="payrolls.last_page > 1"
                class="flex justify-between items-center px-4 py-3 bg-white border-t border-gray-200 sm:px-6"
            >
                <div class="flex-1 flex items-center justify-between">
                    ...
                    <div>
                        ...
                        >
                            <!-- Previous Page -->
                            <button
                                @click="changePage(payrolls.current_page - 1)"
                                :disabled="payrolls.current_page === 1"
                            ...
                            <!-- Page Numbers -->
                            <template
                                v-for="page in Math.min(5, payrolls.last_page)"
                                :key="page"
                            >
                                <button
                            ...
                            <button
                                v-if="
                                    payrolls.last_page > 5 &&
                                    payrolls.current_page <
                                        payrolls.last_page - 2
                                "
                                @click="changePage(payrolls.last_page)"
                                class="relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50"
                            >
                                {{ payrolls.last_page }}
                            </button>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            >
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <button
                                        @click="
                                            openValidationModal(beneficiary)
                                        "
        ...
        >
            <div>
                <p class="text-sm text-gray-700">
                    Showing
                    <span class="font-medium">{{
                        beneficiariesPagination.from || 0
                    }}</span>
                    to
                    <span class="font-medium">{{
                        beneficiariesPagination.to || 0
                    }}</span>
                    of
                    <span class="font-medium">{{
                        beneficiariesPagination.total || 0
                    }}</span>
                    results
                </p>
            </div>
...
const changePage = (page) => {
    // Validate page range
    if (page < 1 || page > props.payrolls.last_page) return;

    // Update form's page and search
    form.page = page;
    ...
    router.get(
        ...
        },
        {
            ...
            onError: (errors) => {
                isLoading.value = false;
                error.value =
                    errors.message || "An error occurred while changing pages.";
            },
        }
    );
};
...
const shouldShowPage = (page) => {
    // Use props.payrolls instead of undefined payrolls variable
    const current = props.payrolls.current_page;
    const last = props.payrolls.last_page;
...
const refreshPayrollData = async () => {
    try {
        const response = await axios.get(
            `/payroll/${selectedPayroll.value.payrollID}`
        );
        selectedPayroll.value = {
            ...response.data.payroll,
        ...
        setError(
            "An error occurred while refreshing payroll data. Please try again."
        );
    }
};
...
const setAmount = async () => {
    ...
    const parsedAmount = parseFloat(
        parseAmount(beneficiaryAmount.value)
    ).toFixed(2);
...
const markAllClaimed = async () => {
    ...
    try {
        await axios.post(
            `/payroll/${selectedPayroll.value.payrollID}/mark-all-claimed`
        );
        await fetchBeneficiaries(selectedPayroll.value.payrollID);
...
const exportPayroll = async () => {
    ...
    try {
        ...
        console.error(err);
        setError(
            "An error occurred while exporting the payroll. Please try again."
        );
    } finally {
