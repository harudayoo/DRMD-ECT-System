<template>
  <div class="container mx-auto px-4 py-8">
    ...
    <div v-if="selectedPayroll" class="mt-8">
      ...
      <!-- Set Amount for Beneficiaries -->
      <div class="mb-4 flex items-center">
        <input
          v-model="beneficiaryAmount"
          type="number"
          placeholder="Set Amount for Beneficiaries"
          class="w-64 px-4 py-2 border rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mr-4"
        />
        <button
          @click="setAmount"
    ...
    <div v-if="beneficiariesPagination" class="mt-6 flex justify-between items-center">
      <div>
        <p class="text-sm text-gray-700">
          Showing
          <span class="font-medium">{{ beneficiariesPagination.from }}</span>
          to
          <span class="font-medium">{{ beneficiariesPagination.to }}</span>
          of
          <span class="font-medium">{{ beneficiariesPagination.total }}</span>
          results
        </p>
      </div>
      <div>
    ...
    <div
      ...
      class="text-center py-8"
    >
      <p class="text-gray-500 text-lg">No beneficiaries found for this payroll.</p>
    </div>
  </div>
</template>
...
const updateBeneficiaryStatus = async (beneficiary) => {
  try {
    ...
    await axios.post(`/payroll/${selectedPayroll.value.payrollID}/beneficiaries`, {
      ...
      beneficiaries: [
        { beneficiaryID: beneficiary.beneficiaryID, status: beneficiary.status },
      ],
    });
    await fetchBeneficiaries(selectedPayroll.value.payrollID);
  } catch (err) {
