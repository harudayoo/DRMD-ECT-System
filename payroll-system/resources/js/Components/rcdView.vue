<template>
  <div class="relative">
    ...
    <div v-if="selectedRCD" class="bg-white shadow mt-4 overflow-hidden sm:rounded-lg">
    ...
    >
      ...
      <span class="absolute top-0 bottom-0 right-0 px-4 py-3" @click="apiError = null">
        <svg
          class="fill-current h-6 w-6 text-red-500"
          role="button"
          xmlns="http://www.w3.org/2000/svg"
...
const updateRCD = async (field, value) => {
  ...
  try {
    ...
    // Process the value
    const actualValue = value?.id || value?.value || value?.target?.value || value;
    ...
    if (response.data.success) {
      ...
      showNotification("Success", `Successfully updated ${field}`, "success");
    } else {
...
const handleUpdateError = (error, field) => {
  ...
  showNotification("Error", `Failed to update ${field}: ${errorMessage}`, "error");
