      ...
      <div class="p-4 relative">
        ...
        <ul class="mt-4 ml-4 font-thin">
          ...
          <li class="mb-4 text-xl">
            ...
            >
              <a href="#" @click="openModal" class="block text-xl hover:text-gray-400"
                >Add</a
              >
              <a href="#" class="block text-xl hover:text-gray-400">Edit</a>
            </div>
          </li>
          ...
          <li class="mb-4 text-xl">
            ...
            <div v-show="openSubMenu === 'masterlist'" class="mt-2 ml-4 rounded-md p-1">
              <a href="#" class="block text-xl hover:text-gray-400">Import</a>
              <a href="#" class="block text-xl hover:text-gray-400">Export</a>
            </div>
          </li>
          ...
          <li class="mb-4 text-xl">
            ...
            <div v-show="openSubMenu === 'documents'" class="mt-2 ml-4 rounded-md p-1">
              <a href="#" class="block text-xl hover:text-gray-400">Payroll Form</a>
              <a href="#" class="block text-xl hover:text-gray-400">Cash Report</a>
              <a href="#" class="block text-xl hover:text-gray-400">RCD</a>
              <a href="#" class="block text-xl hover:text-gray-400">CDR</a>
            </div>
          </li>
        </ul>
      </div>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <div class="ml-1 mt-1 mx-auto py-2 px-4 flex items-center">
          ...
          <div>
            <h1 class="text-2xl font-black text-black">
              {{ provinceName || "Province Name Not Set" }}
            </h1>

            <h2 class="text-lg -mt-1 font-medium">
              Number of Beneficiaries and Assistance Provided per Municipality
            </h2>
          </div>
        </div>
      </header>
      ...
      <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <div class="flex mx-4 mb-4 h-2/6">
          <!-- Statistics -->
          <div class="text-black shadow-2xl -mt-14 text-lg rounded-xl w-1/4 p-2">
            <h2
            ...
            >
              <span>Unclaimed:</span>
              <span class="font-medium">{{ totalUnclaimed }}</span>
            </div>
            <div
              class="flex justify-between items-center font-medium py-1 px-2 border-t border-b border-white"
            >
              <span>Disqualified:</span>
              <span class="font-medium">{{ totalDisqualified }}</span>
            </div>
            <div
              class="flex justify-between items-center font-medium py-1 px-2 border-t border-b border-white"
            >
              <span>Duplicates:</span>
              <span class="font-medium">{{ totalDoubleEntry }}</span>
            </div>
          </div>

          <!-- Main Content -->
          <div class="flex-1 border shadow-2xl rounded-xl p-2 -mt-14 ml-2 flex flex-col">
              ...
              <thead>
                <tr>
                  <th
                    ...
                    :class="{
                      'w-1/5 text-left rounded-tl-xl': index === 0,
                      'w-3/5 text-center': index === 1,
                      'w-1/4 text-right rounded-tr-xl': index === tableHeaders.length - 1,
                    }"
                  >
                    <div
                      class="h-full flexitems-center"
                      :class="{
            ...
            <div class="overflow-y-auto flex-grow" style="max-height: 180px">
              <table class="w-full divide-y divide-gray-200">
                <tbody class="bg-white divide-y text-lg font-medium divide-gray-200">
                  <tr
                    v-for="municipality in municipalities"
                    :key="municipality.municipalityID"
                    @click="goToBarangays(municipality.municipalityID)"
                    class="cursor-pointer hover:bg-gray-200"
                  >
                    <td class="px-2 py-1 whitespace-nowrap w-1/5">
                      <div class="flex justify-between items-center">
                        <span class="truncate mr-2" style="max-width: calc(100% - 24px)">
                    ...
                    <td class="px-2 py-1 whitespace-nowrap w-3/5 text-center">
                      {{ municipality.totalBeneficiaries }}
                    </td>
                    <td class="px-2 py-1 whitespace-nowrap w-1/4 text-right">
                      ₱{{ parseFloat(municipality.totalAmountReleased).toFixed(2) }}
                    </td>
                  </tr>
                </tbody>
              </table>
...
const goToBarangays = (municipalityID) => {
...
const totalUnclaimed = computed(() =>
  props.municipalities.reduce((sum, municipality) => sum + municipality.unclaimed, 0)
);

const totalDisqualified = computed(() =>
  props.municipalities.reduce((sum, municipality) => sum + municipality.disqualified, 0)
);

const totalDoubleEntry = computed(() =>
  props.municipalities.reduce((sum, municipality) => sum + municipality.double_entry, 0)
);
...
const totalAmount = computed(() => {
  return props.municipalities
    .reduce((sum, municipality) => sum + parseFloat(municipality.totalAmountReleased), 0)
    .toLocaleString("en-PH", { style: "currency", currency: "PHP" });
});

// Chart data and configuration
const labels = computed(() => props.municipalities.map((m) => m.municipalityName));
...
const data = computed(() => ({
  ...
  datasets: [
    ...
    },
    {
      ...
      data: props.municipalities.map((m) => parseFloat(m.totalAmountReleased)),
    },
  ],
}));
