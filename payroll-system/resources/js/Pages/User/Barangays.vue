      ...
      <div class="p-4 relative">
        ...
        <ul class="mt-4 ml-4 font-thin">
          ...
          <li class="mb-4 text-xl">
            ...
            >
              <a href="/add-beneficiary" class="block text-xl hover:text-gray-400">Add</a>
        ...
        <div class="ml-1 mt-1 mx-auto py-2 px-4 flex items-center">
          ...
          <div>
            ...
            <h2 class="text-lg -mt-1 font-medium">
              Number of Beneficiaries and Assistance Provided per Barangay
            </h2>
          </div>
        </div>
      </header>
      ...
      <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <div class="flex mx-4 mb-4 h-2/6">
          ...
          <div class="text-black shadow-2xl -mt-14 text-lg rounded-xl w-1/4 p-2">
            ...
            <div
              v-for="(value, key) in statusAnalytics"
              :key="key"
              class="flex justify-between items-center font-medium py-1 px-2 border-t border-white"
              :class="{
                'border-b': key === Object.keys(statusAnalytics).length - 1,
              }"
            >
              ...
              <table class="w-full divide-y divide-gray-200">
                <tbody class="bg-white divide-y text-lg font-medium divide-gray-200">
                  <tr
                    ...
                    @click="navigateToBarangay(barangay.barangayID)"
                  ...
                  >
                    ...
                    <td class="px-2 py-1 whitespace-nowrap w-3/5 text-center">
                      {{ barangay.totalBeneficiaries }}
                    </td>
                    <td class="px-2 py-1 whitespace-nowrap w-1/4 text-right">
                      ₱{{ parseFloat(barangay.totalAmountReleased).toFixed(2) }}
                    </td>
                  </tr>
                </tbody>
              </table>
...
const formatLabel = (key) => {
  const labels = {
    ...
    doubleEntry: "Duplicates",
  };
  return labels[key] || key.charAt(0).toUpperCase() + key.slice(1);
};

const tableHeaders = ref(["Barangay", "Number of Beneficiary", "Amount"]);

const totalAmount = computed(() => {
  return props.barangays
    .reduce((sum, barangay) => sum + parseFloat(barangay.totalAmountReleased), 0)
    .toLocaleString("en-PH", { style: "currency", currency: "PHP" });
});

// Chart data and configuration
const labels = computed(() => props.barangays.map((b) => b.barangayName));
  ...
  datasets: [
    {
      ...
      data: props.barangays.map((b) => b.totalBeneficiaries),
    },
    {
      label: "Amount Released",
      backgroundColor: "rgb(166, 170, 238)",
      data: props.barangays.map((b) => parseFloat(b.totalAmountReleased)),
    },
  ],
}));
